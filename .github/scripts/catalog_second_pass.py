...
def main():
        ...
        if len(valid) == 1:
            ...
            for duplicate in erroneous:
                for key, value in duplicate.items():
                    if target.get(key) in (None, '') and value not in (None, ''): target[key] = value
                vehicles.remove(duplicate)
            print('Removed erroneous SEALION 7 Excellence AWD 91.5 kWh record(s):', len(erroneous))
        elif len(erroneous) == 1 and len(sealion_excellence) == 1:
    ...
    if len(vehicles) != original - (len(erroneous) if len(valid) == 1 else 0): raise RuntimeError('Unexpected vehicle count after SEALION cleanup')
...
# Force a fresh final audit run after the catalog deduplication corrections.
