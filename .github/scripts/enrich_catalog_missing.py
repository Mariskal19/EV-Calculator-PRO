...
EAFO_BASE = 'https://alternative-fuels-observatory.ec.europa.eu/markets-and-policy/market-and-consumer-insights/available-electric-vehicle-models?page={}'
...
def candidate_fields(v):
    ...
    battery=first(b,'pack_capacity_kwh_net','capacity_kwh'); power=first(p,'system_power_kw','power_kw'); drive=first(p,'drivetrain') or v.get('drive_type'); accel=first(perf,'acceleration_0_100_kmh_s','acceleration_0_100_kmh') or v.get('acceleration_0_100_kmh')
    ...
    if isinstance(ct,dict):
        for row in ct.get('dc',[]):
            if isinstance(row,dict) and num(row.get('from_soc_percent'))==10 and num(row.get('to_soc_percent'))==80: charge_time=row.get('time_min'); break
...
class TableParser(HTMLParser):
    def __init__(self):
        super().__init__(); self.in_tr=False; self.in_cell=False; self.rows=[]; self.row=[]; self.buf=[]
...
def load_eafo():
    ...
    for page in range(0,20):
        req=Request(EAFO_BASE.format(page),headers={'User-Agent':'EV-Calculator-PRO catalog research'})
        try:
            with urlopen(req,timeout=30) as r: html=r.read().decode('utf-8','ignore')
        except Exception as exc:
            print(f'EAFO page {page}: skipped ({exc})'); break
        p=TableParser(); p.feed(html)
        page_rows=[r for r in p.rows if len(r)>=7 and r[0] not in ('Model','')]
        if not page_rows: break
        rows.extend(page_rows)
    out=[]
    for r in rows:
        # Expected: Model, Available from, Range, Battery size, Efficiency, Fastcharge speed, Price
        if len(r)<4: continue
        model=r[0]; m=re.search(r'(\d{2})-(\d{4})',r[1]); year=int(m.group(2)) if m else None
        batt=num(re.search(r'([0-9]+(?:\.[0-9]+)?)',r[3]).group(1)) if re.search(r'([0-9]+(?:\.[0-9]+)?)',r[3]) else None
        if year and batt: out.append({'model':model,'year':year,'battery':batt})
    print(f'EAFO rows loaded: {len(out)}')
    return out
...
def score(target,src):
    ...
    c=candidate_fields(src); s+=10; tv=num(target.get('powerKw'))
    ...
    if version and variant: s+=min(6,len(set(re.findall(r'[a-z0-9]+',version))&set(re.findall(r'[a-z0-9]+',variant))))
    return s
...
def eafo_score(v,row):
    text=norm(row['model']); make=norm(v.get('make')); model=norm(v.get('model')); tokens=set(model.split()); rt=set(text.split())
    if make not in text or not (model==text or tokens.issubset(rt) or rt.issubset(tokens)): return -999
    year=int(v.get('year',0));
    if row['year']>year: return -999
    batt=num(v.get('batteryKwh'))
    if batt is None: return 0
    diff=abs(batt-row['battery'])
    if diff>max(8,batt*0.12): return -999
    version=norm(v.get('version')); overlap=len(set(version.split())&rt)
    return 20-overlap*0.1-diff

def main():
    source_vehicles=load_source(); eafo=load_eafo(); catalog=json.loads(CATALOG.read_text(encoding='utf-8')); vehicles=catalog.get('vehicles')
    if not isinstance(vehicles,list): raise RuntimeError('Catalog vehicles array missing')
    original_count=len(vehicles); changes=matched=ambiguous=eafo_added=0; by_brand={}; unmatched={}
    for v in vehicles:
        scored=sorted(((score(v,x),x) for x in source_vehicles if score(v,x)>=18),key=lambda z:z[0],reverse=True)
        if scored and not (len(scored)>1 and scored[0][0]==scored[1][0]):
        ...
        elif scored: ambiguous+=1
        if not v.get('usableBatteryKwh'):
            er=sorted(((eafo_score(v,r),r) for r in eafo if eafo_score(v,r)>=10),key=lambda z:z[0],reverse=True)
            if er and not (len(er)>1 and abs(er[0][0]-er[1][0])<0.25):
                if set_missing(v,'usableBatteryKwh',er[0][1]['battery']):
                    eafo_added+=1
                    if not v.get('source'): v['source']=f'European Alternative Fuels Observatory (EAFO) — available electric vehicle models'
                    if not v.get('lastUpdated'): v['lastUpdated']=TODAY
        if not v.get('usableBatteryKwh') and not scored: unmatched[(v.get('make'),v.get('model'))]=unmatched.get((v.get('make'),v.get('model')),0)+1
    if len(vehicles)!=original_count: raise RuntimeError('Vehicle count changed unexpectedly')
    CATALOG.write_text(json.dumps(catalog,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    print(f'Catalog vehicles: {original_count}; OpenEV matched: {matched}; ambiguous: {ambiguous}; OpenEV fields filled: {changes}; EAFO net-battery fields filled: {eafo_added}')
    for brand,count in sorted(by_brand.items()): print(f'  {brand}: +{count}')
    print(f'Remaining unmatched without net battery: {len(unmatched)} model groups')
