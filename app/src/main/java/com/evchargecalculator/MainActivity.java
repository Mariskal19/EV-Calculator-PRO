...
public class MainActivity extends Activity {
 ...
 void build(){
  ...
  LinearLayout timeRow=new LinearLayout(this);timeRow.setGravity(Gravity.CENTER_VERTICAL);timeRow.setPadding(0,0,dp(34),0);
  ...
  timeRow.addView(h3,new LinearLayout.LayoutParams(0,dp(54),1));
  timeR=tv("00 h 00 min",30,text());timeR.setTypeface(null,1);timeR.setGravity(Gravity.CENTER_VERTICAL|Gravity.END);timeR.setSingleLine(true);timeR.setIncludeFontPadding(false);
  timeRow.addView(timeR,new LinearLayout.LayoutParams(0,dp(54),1));
  timeContainer.addView(timeRow,new FrameLayout.LayoutParams(-1,dp(54)));
  ...
  c3.addView(timeContainer,new LinearLayout.LayoutParams(-1,dp(54)));
  spaceIn(c3,6);
  LinearLayout costRow=new LinearLayout(this);costRow.setGravity(Gravity.CENTER_VERTICAL);
  TextView costLabel=tv("Coste de carga",14,sub());costLabel.setGravity(Gravity.CENTER_VERTICAL|Gravity.START);costLabel.setIncludeFontPadding(false);costRow.addView(costLabel,new LinearLayout.LayoutParams(0,dp(42),1));
  costR=tv("0,00 € (0,0 kWh)",16,sub());costR.setGravity(Gravity.CENTER_VERTICAL|Gravity.END);costR.setSingleLine(true);costR.setIncludeFontPadding(false);costRow.addView(costR,new LinearLayout.LayoutParams(0,dp(42),1));c3.addView(costRow);
  ...
  spaceIn(c4,18);c4.addView(statusBox,new LinearLayout.LayoutParams(-1,dp(78)));root.addView(c4);space(10);String appVersion="1.0.19";try{appVersion=getPackageManager().getPackageInfo(getPackageName(),0).versionName;}catch(Exception ignored){}TextView foot=tv("Powered by EV Charge Calculator · v"+appVersion,12,sub());foot.setGravity(Gravity.CENTER);root.addView(foot);space(8);setup();
 }
