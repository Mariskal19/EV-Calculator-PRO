...
public class MainActivity extends Activity {
 ...
 void build(){
  ...
  costR=tv("0,00 € (0,0 kWh)",16,sub());costR.setGravity(Gravity.CENTER_VERTICAL|Gravity.END);costR.setSingleLine(true);costR.setIncludeFontPadding(false);LinearLayout.LayoutParams costValueLp=new LinearLayout.LayoutParams(0,dp(42),1);costValueLp.setMargins(0,0,dp(12),0);costRow.addView(costR,costValueLp);c3.addView(costRow);
  ...
  spaceIn(c4,18);c4.addView(statusBox,new LinearLayout.LayoutParams(-1,dp(78)));root.addView(c4);space(10);String appVersion="1.0.21";try{appVersion=getPackageManager().getPackageInfo(getPackageName(),0).versionName;}catch(Exception ignored){}TextView foot=tv("Powered by EV Charge Calculator · v"+appVersion,12,sub());foot.setGravity(Gravity.CENTER);root.addView(foot);space(8);setup();
 }
