...
public class MainActivity extends Activity {
 ...
 int blue=Color.rgb(46,107,255), white=Color.rgb(22,42,63), secondary=Color.rgb(90,111,137), cardLight=Color.argb(245,255,255,255), cardDark=Color.argb(220,21,31,42), borderLight=Color.rgb(217,228,241), headerLight=Color.argb(248,250,255,255), headerDark=Color.argb(190,17,29,42), darkBg=Color.rgb(7,19,28), darkCard=Color.rgb(21,35,50), statusOk=Color.rgb(29,142,121), statusWarn=Color.rgb(227,93,93);
 ...
 GradientDrawable bg(int color,float r,int stroke){GradientDrawable g=new GradientDrawable();g.setColor(color);g.setCornerRadius(r);if(stroke>0)g.setStroke(1,dark?Color.rgb(38,59,85):borderLight);return g;}
 ...
 void build(){
  ...
  scroll=new ScrollView(this);scroll.setFillViewport(true);root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(0,dp(36),0,dp(68));scroll.addView(root);frame.addView(scroll,new FrameLayout.LayoutParams(-1,-1));setContentView(frame);
  ...
  FrameLayout.LayoutParams infoLp=new FrameLayout.LayoutParams(dp(30),dp(30),Gravity.TOP|Gravity.END);infoLp.setMargins(0,dp(2),-dp(4),0);
  ...
  LinearLayout.LayoutParams costRowLp=new LinearLayout.LayoutParams(-1,dp(20));costRowLp.topMargin=dp(8);costRowLp.bottomMargin=dp(2);
  TextView costLabel=tv("Coste de carga",13,sub());costLabel.setGravity(Gravity.CENTER_VERTICAL|Gravity.START);costLabel.setIncludeFontPadding(false);costRow.addView(costLabel,new LinearLayout.LayoutParams(0,dp(20),1));
  ...
  statusBox=new LinearLayout(this);statusBox.setOrientation(LinearLayout.HORIZONTAL);statusBox.setGravity(Gravity.CENTER_VERTICAL);statusBox.setPadding(dp(10),dp(14),dp(10),dp(14));statusBox.setBackground(bg(dark?Color.rgb(31,93,168):Color.rgb(46,107,255),20,0));
  ...
  spaceIn(c4,10);c4.addView(statusBox,new LinearLayout.LayoutParams(-1,dp(66)));root.addView(c4);space(10);
  String appVersion="1.0.1";try{appVersion=getPackageManager().getPackageInfo(getPackageName(),0).versionName;}catch(Exception ignored){}if(appVersion.startsWith("v")||appVersion.startsWith("V")){appVersion=appVersion.substring(1);}
  TextView privacyLink=tv("Política de privacidad",13,dark?Color.rgb(105,175,255):blue);privacyLink.setGravity(Gravity.CENTER);privacyLink.setTypeface(null,1);privacyLink.setPadding(0,dp(4),0,dp(4));privacyLink.setClickable(true);privacyLink.setFocusable(true);privacyLink.setContentDescription("Política de privacidad");privacyLink.setOnClickListener(v->{Intent intent=new Intent(Intent.ACTION_VIEW, Uri.parse(PRIVACY_URL));startActivity(intent);});root.addView(privacyLink,new LinearLayout.LayoutParams(-1,dp(34)));
  TextView foot=tv("Powered by EV Calculator · v"+appVersion,12,sub());foot.setGravity(Gravity.CENTER);root.addView(foot,new LinearLayout.LayoutParams(-1,dp(28)));space(8);setup(); }
 ...
 void applyTheme(){background.setDark(dark); if(range!=null) range.setDark(dark); getWindow().setStatusBarColor(dark?darkBg:Color.rgb(244,248,255)); getWindow().setNavigationBarColor(dark?darkBg:Color.rgb(244,248,255)); getWindow().getDecorView().setSystemUiVisibility(dark?0:View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR); themeButton.setText(dark?"☀":"☾"); themeButton.setTextColor(dark?Color.WHITE:blue); themeButton.setBackground(bg(dark?Color.argb(18,255,255,255):Color.argb(22,46,107,255),12,1));}
 ...
 String fmtBatteryValue(double v){if(Math.abs(v-Math.round(v))<1e-9){return String.valueOf((int)Math.round(v));}return fmt(v,1);} 
 ...
 void statusBoxLayout(boolean ok, boolean moreThan24){
  ...
  if(ok){ GradientDrawable g = new GradientDrawable(GradientDrawable.Orientation.LEFT_RIGHT, new int[]{Color.rgb(18,82,214),Color.rgb(54,184,255)}); g.setCornerRadius(dp(22)); statusBox.setBackground(g); } else { GradientDrawable g = new GradientDrawable(GradientDrawable.Orientation.LEFT_RIGHT, new int[]{Color.rgb(190,38,52),Color.rgb(255,92,92)}); g.setCornerRadius(dp(22)); statusBox.setBackground(g); }
