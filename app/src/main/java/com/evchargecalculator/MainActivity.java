...
public class MainActivity extends Activity {
 ...
 void build(){
  ...
  spaceIn(c4,10);c4.addView(statusBox,new LinearLayout.LayoutParams(-1,dp(66)));root.addView(c4);space(10);  String appVersion="1.0.1";try{appVersion=getPackageManager().getPackageInfo(getPackageName(),0).versionName;}catch(Exception ignored){}if(appVersion.startsWith("v")||appVersion.startsWith("V")){appVersion=appVersion.substring(1);}TextView foot=tv("Powered by EV Calculator · v"+appVersion,12,sub());foot.setGravity(Gravity.CENTER);root.addView(foot);space(8);setup(); }
