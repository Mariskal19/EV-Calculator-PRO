...
public class MainActivity extends Activity {
 ...
 void build(){
  ...
  LinearLayout c3=card();TextView h3=tv("Resultado",18,text());h3.setTypeface(null,1);c3.addView(h3);spaceIn(c3,14);
  LinearLayout timeRow=new LinearLayout(this);timeRow.setGravity(Gravity.CENTER_VERTICAL);TextView timeLabel=tv("Tiempo de Carga",16,text());timeRow.addView(timeLabel,new LinearLayout.LayoutParams(0,dp(54),1));timeR=tv("00 h 00 min",30,text());timeR.setTypeface(null,1);timeR.setGravity(Gravity.CENTER_VERTICAL|Gravity.END);timeR.setSingleLine(true);timeRow.addView(timeR,new LinearLayout.LayoutParams(-2,dp(54)));c3.addView(timeRow);
  LinearLayout costRow=new LinearLayout(this);costRow.setGravity(Gravity.CENTER_VERTICAL);TextView costLabel=tv("Coste de carga",16,text());costRow.addView(costLabel,new LinearLayout.LayoutParams(0,dp(44),1));costR=tv("0,00 € (0,0 kWh)",16,sub());costR.setGravity(Gravity.CENTER_VERTICAL|Gravity.END);costR.setSingleLine(true);costRow.addView(costR,new LinearLayout.LayoutParams(-2,dp(44)));c3.addView(costRow);energyR=tv("",1,sub());energyR.setVisibility(View.GONE);c3.addView(energyR);root.addView(c3);space(14);
  LinearLayout c4=card();LinearLayout departureHeader=new LinearLayout(this);departureHeader.setGravity(Gravity.CENTER_VERTICAL);TextView h4=tv("Hora Salida",18,text());h4.setTypeface(null,1);h4.setIncludeFontPadding(false);h4.setGravity(Gravity.CENTER_VERTICAL);departureHeader.addView(h4,new LinearLayout.LayoutParams(0,54,1));departure=edit("07:00");departure.setTypeface(null,1);departure.setTextSize(17);departure.setInputType(android.text.InputType.TYPE_CLASS_DATETIME|android.text.InputType.TYPE_DATETIME_VARIATION_TIME);departure.setKeyListener(null);departure.setCursorVisible(false);departure.setShowSoftInputOnFocus(false);departure.setSelectAllOnFocus(false);departure.setFocusable(false);departure.setClickable(true);departure.setOnClickListener(v->{hideKeyboard(v);pickTime(departure);});departureHeader.addView(departure,new LinearLayout.LayoutParams(dp(88),54));c4.addView(departureHeader);
  ...
  spaceIn(c4,18);c4.addView(statusBox,new LinearLayout.LayoutParams(-1,dp(78)));root.addView(c4);space(10);String appVersion="1.0.15";try{appVersion=getPackageManager().getPackageInfo(getPackageName(),0).versionName;}catch(Exception ignored){}TextView foot=tv("Powered by EV Charge Calculator · v"+appVersion,12,sub());foot.setGravity(Gravity.CENTER);root.addView(foot);space(8);setup();
 }
