...
public class MainActivity extends Activity {
 ...
 void build(){
  ...
  c3.addView(timeContainer,new LinearLayout.LayoutParams(-1,dp(98)));
  spaceIn(c3,1);
  LinearLayout costRow=new LinearLayout(this);costRow.setGravity(Gravity.CENTER_VERTICAL);
  LinearLayout.LayoutParams costRowLp=new LinearLayout.LayoutParams(-1,dp(28));costRowLp.topMargin=-dp(8);costRowLp.bottomMargin=dp(1);
  ...
  spaceIn(c4,10);c4.addView(statusBox,new LinearLayout.LayoutParams(-1,dp(66)));root.addView(c4);space(10);String appVersion="1.0.22";try{appVersion=getPackageManager().getPackageInfo(getPackageName(),0).versionName;}catch(Exception ignored){}TextView foot=tv("Powered by EV Charge Calculator · v"+appVersion,12,sub());foot.setGravity(Gravity.CENTER);root.addView(foot);space(8);setup();
 }
