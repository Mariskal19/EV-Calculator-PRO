...
public class ConfigurationActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        if(prefs.contains(KEY_DARK_THEME)){dark=prefs.getBoolean(KEY_DARK_THEME,false);}else{dark=(getResources().getConfiguration().uiMode&0x30)==0x20;prefs.edit().putBoolean(KEY_DARK_THEME,dark).putString(KEY_THEME_MODE,dark?"dark":"light").apply();}
    ...
    private void build(){
        ...
        TextView title=tv("⚙  "+tr("Configuración","Settings","Configuration","Einstellungen","Impostazioni","Definições"),22,headerColor);title.setTypeface(null,1);title.setGravity(Gravity.CENTER);title.setTextAlignment(View.TEXT_ALIGNMENT_CENTER);title.setShadowLayer(dp(4),0,dp(2),dark?Color.argb(90,0,0,0):Color.argb(55,0,0,0));FrameLayout.LayoutParams tp=new FrameLayout.LayoutParams(-1,dp(48),Gravity.TOP|Gravity.CENTER_HORIZONTAL);tp.leftMargin=dp(48);tp.rightMargin=dp(48);tp.topMargin=dp(12);hero.addView(title,tp);root.addView(hero);
        LinearLayout card=new LinearLayout(this);card.setOrientation(LinearLayout.VERTICAL);card.setPadding(dp(18),dp(18),dp(18),dp(18));card.setBackground(bg(dark?Color.argb(220,21,31,42):Color.argb(245,255,255,255),22,1));LinearLayout.LayoutParams cp=new LinearLayout.LayoutParams(-1,-2);cp.setMargins(dp(12),-dp(26),dp(12),dp(14));card.setLayoutParams(cp);
        ...
        root.addView(card);Space bottom=new Space(this);root.addView(bottom,new LinearLayout.LayoutParams(1,0,1));
    ...
    private void chooseTheme(){String[] values={"light","dark"};String[] names={tr("Claro","Light","Clair","Hell","Chiaro","Claro"),tr("Oscuro","Dark","Sombre","Dunkel","Scuro","Escuro")};String cur=prefs.getString(KEY_THEME_MODE,dark?"dark":"light");int checked="dark".equals(cur)?1:0;new AlertDialog.Builder(this).setTitle(tr("Tema","Theme","Thème","Design","Tema","Tema")).setSingleChoiceItems(names,checked,(d,w)->{setTheme(values[w]);d.dismiss();}).show();}
    private void setTheme(String mode){dark="dark".equals(mode);prefs.edit().putString(KEY_THEME_MODE,mode).putBoolean(KEY_DARK_THEME,dark).apply();recreate();}
    private String themeName(){String m=prefs.getString(KEY_THEME_MODE,dark?"dark":"light");return "dark".equals(m)?tr("Oscuro","Dark","Sombre","Dunkel","Scuro","Escuro"):tr("Claro","Light","Clair","Hell","Chiaro","Claro");}
