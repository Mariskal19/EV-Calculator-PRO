...
public class ConfigurationActivity extends Activity {
  ...
  public void onCreate(Bundle b) {
    ...
    if (prefs.contains(KEY_DARK_THEME)) {
      ...
      dark = (getResources().getConfiguration().uiMode & 0x30) == 0x20;
      prefs
          .edit()
  ...
  private GradientDrawable bg(int color, float r, int stroke) {
    ...
    if (stroke > 0) g.setStroke(dp(1), dark ? Color.rgb(38, 59, 85) : Color.rgb(217, 228, 241));
    return g;
  }
  ...
  private String lang() {
    return LanguageManager.getSelectedLanguage(this);
  }
  ...
  private void build() {
    ...
    hero.setLayoutParams(new LinearLayout.LayoutParams(-1, dp(220)));
    View glow = new View(this);
    GradientDrawable gd =
        new GradientDrawable(
            GradientDrawable.Orientation.TL_BR,
            new int[] {
              dark ? Color.rgb(8, 34, 58) : Color.rgb(231, 244, 255),
              dark ? Color.rgb(15, 69, 110) : Color.rgb(195, 229, 255),
              dark ? Color.rgb(5, 25, 42) : Color.rgb(242, 250, 255)
            });
    ...
    title.setShadowLayer(dp(4), 0, dp(2), dark ? Color.argb(90, 0, 0, 0) : Color.argb(55, 0, 0, 0));
    FrameLayout.LayoutParams tp =
        new FrameLayout.LayoutParams(-1, dp(48), Gravity.TOP | Gravity.CENTER_HORIZONTAL);
    tp.leftMargin = dp(48);
    tp.rightMargin = dp(48);
    tp.topMargin = dp(12);
    ...
    card.setBackground(
        bg(dark ? Color.argb(220, 21, 31, 42) : Color.argb(245, 255, 255, 255), 22, 1));
    LinearLayout.LayoutParams cp = new LinearLayout.LayoutParams(-1, -2);
    cp.setMargins(dp(12), -dp(26), dp(12), dp(14));
    ...
    languageValue =
        addRow(
            card,
            "🌐",
            tr("Idioma", "Language", "Langue", "Sprache", "Lingua", "Idioma"),
            LanguageManager.displayName(lang()),
            v -> chooseLanguage());
    currencyValue =
        addRow(
            card,
            "💰",
            tr("Moneda", "Currency", "Devise", "Währung", "Valuta", "Moeda"),
            currencyName(prefs.getString(KEY_CURRENCY, "EUR")),
            v -> chooseCurrency());
    themeValue =
        addRow(
            card,
            "🎨",
            tr("Tema", "Theme", "Thème", "Design", "Tema", "Tema"),
            themeName(),
            v -> chooseTheme());
    root.addView(card);
    ...
    TextView privacyLink =
        tv(
            ...
            13,
            dark ? Color.rgb(105, 175, 255) : Color.rgb(46, 107, 255));
  ...
  private TextView addRow(
      LinearLayout parent, String icon, String label, String value, View.OnClickListener click) {
    LinearLayout r = new LinearLayout(this);
    r.setGravity(Gravity.CENTER_VERTICAL);
    r.setPadding(dp(6), dp(8), dp(4), dp(8));
    r.setBackground(bg(dark ? Color.rgb(21, 34, 51) : Color.rgb(246, 249, 253), 16, 1));
    TextView i = tv(icon, 22, text());
    i.setGravity(Gravity.CENTER);
    r.addView(i, new LinearLayout.LayoutParams(dp(44), dp(62)));
    LinearLayout texts = new LinearLayout(this);
    texts.setOrientation(LinearLayout.VERTICAL);
    texts.setGravity(Gravity.CENTER_VERTICAL);
    TextView l = tv(label, 15, text());
    l.setTypeface(null, 1);
    texts.addView(l);
    TextView v = tv(value, 13, sub());
    v.setTag("value");
    texts.addView(v);
    r.addView(texts, new LinearLayout.LayoutParams(0, dp(62), 1));
    TextView arrow = tv("›", 30, sub());
    arrow.setGravity(Gravity.CENTER);
    r.addView(arrow, new LinearLayout.LayoutParams(dp(36), dp(62)));
  ...
  private void chooseLanguage() {
    String[] codes = {"en", "es", "fr", "de", "it", "pt"};
    String[] names = {
      "🇬🇧  English",
      "🇪🇸  Español",
      "🇫🇷  Français",
    ...
    new AlertDialog.Builder(this)
        .setTitle(tr("Idioma", "Language", "Langue", "Sprache", "Lingua", "Idioma"))
  ...
  private void chooseCurrency() {
    String[] codes = {"EUR", "USD", "GBP", "CHF", "CAD", "AUD"};
    String[] names = {
      "€  EUR — Euro",
    ...
    String cur = prefs.getString(KEY_CURRENCY, "EUR");
    int checked = 0;
    for (int i = 0; i < codes.length; i++) if (codes[i].equals(cur)) checked = i;
    new AlertDialog.Builder(this)
        .setTitle(tr("Moneda", "Currency", "Devise", "Währung", "Valuta", "Moeda"))
        .setSingleChoiceItems(
            names,
            checked,
            (d, w) -> {
              prefs.edit().putString(KEY_CURRENCY, codes[w]).apply();
              d.dismiss();
              currencyValue.setText(currencyName(codes[w]));
            })
        .show();
  }
  ...
  private void chooseTheme() {
    ...
    String[] names = {
      tr("Claro", "Light", "Clair", "Hell", "Chiaro", "Claro"),
      tr("Oscuro", "Dark", "Sombre", "Dunkel", "Scuro", "Escuro")
    };
    String cur = prefs.getString(KEY_THEME_MODE, dark ? "dark" : "light");
    int checked = "dark".equals(cur) ? 1 : 0;
    new AlertDialog.Builder(this)
        .setTitle(tr("Tema", "Theme", "Thème", "Design", "Tema", "Tema"))
        .setSingleChoiceItems(
            names,
            checked,
            (d, w) -> {
  ...
  private void setTheme(String mode) {
    dark = "dark".equals(mode);
    prefs.edit().putString(KEY_THEME_MODE, mode).putBoolean(KEY_DARK_THEME, dark).apply();
    recreate();
  }

  private String themeName() {
    String m = prefs.getString(KEY_THEME_MODE, dark ? "dark" : "light");
    return "dark".equals(m)
        ? tr("Oscuro", "Dark", "Sombre", "Dunkel", "Scuro", "Escuro")
        : tr("Claro", "Light", "Clair", "Hell", "Chiaro", "Claro");
  }

  private String currencyName(String c) {
    if ("USD".equals(c)) return "$  USD";
    if ("GBP".equals(c)) return "£  GBP";
    if ("CHF".equals(c)) return "CHF";
    if ("CAD".equals(c)) return "CA$  CAD";
    if ("AUD".equals(c)) return "A$  AUD";
    return "€  EUR";
  }
