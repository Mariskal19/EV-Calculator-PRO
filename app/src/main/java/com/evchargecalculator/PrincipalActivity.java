...
public class PrincipalActivity extends Activity {
    ...
    private void build(){
        ...
        TextView privacy=tv("Política de privacidad",13,dark?Color.rgb(105,175,255):blue);privacy.setGravity(Gravity.CENTER);privacy.setTypeface(null,1);privacy.setPadding(0,dp(4),0,dp(4));privacy.setContentDescription(LanguageManager.t(this,"Política de privacidad"));privacy.setOnClickListener(v->startActivity(new Intent(Intent.ACTION_VIEW,Uri.parse(PRIVACY_URL))));LinearLayout.LayoutParams privacyLp=new LinearLayout.LayoutParams(-1,dp(34));privacyLp.topMargin=dp(180);root.addView(privacy,privacyLp);String ver="1.0.2";try{ver=getPackageManager().getPackageInfo(getPackageName(),0).versionName;}catch(Exception ignored){}if(ver.startsWith("v")||ver.startsWith("V"))ver=ver.substring(1);TextView foot=tv("Powered by EV Calculator · v"+ver,12,subColor());foot.setGravity(Gravity.CENTER);root.addView(foot,new LinearLayout.LayoutParams(-1,dp(28)));getWindow().setStatusBarColor(dark?darkBg:lightBg);getWindow().setNavigationBarColor(dark?darkBg:lightBg);getWindow().getDecorView().setSystemUiVisibility(dark?0:View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
    }
