...
public class PrincipalActivity extends Activity {
    ...
    private void build(){
        ...
        LinearLayout headerRow=new LinearLayout(this);headerRow.setOrientation(LinearLayout.HORIZONTAL);headerRow.setGravity(Gravity.CENTER_VERTICAL);headerRow.setPadding(dp(40),0,dp(14),0);
        TextView title=tv("EV Calculator PRO",22,Color.WHITE);title.setTypeface(null,1);title.setGravity(Gravity.CENTER);title.setIncludeFontPadding(false);title.setTextAlignment(View.TEXT_ALIGNMENT_CENTER);title.setShadowLayer(dp(4),0,dp(2),Color.argb(90,0,0,0));headerRow.addView(title,new LinearLayout.LayoutParams(0,dp(48),1));
        TextView menuButton=tv("←",30,Color.WHITE);menuButton.setGravity(Gravity.CENTER);menuButton.setIncludeFontPadding(false);menuButton.setTextAlignment(View.TEXT_ALIGNMENT_CENTER);menuButton.setContentDescription("Más opciones");menuButton.setPadding(0,0,0,0);menuButton.setShadowLayer(dp(4),0,dp(2),Color.argb(90,0,0,0));menuButton.setBackgroundColor(Color.TRANSPARENT);menuButton.setOnClickListener(v->AppMenuHelper.show(this,v,new AppMenuHelper.Listener(){public boolean isDark(){return dark;}public void setDark(boolean d){dark=d;getSharedPreferences(PREFS,MODE_PRIVATE).edit().putBoolean(KEY_DARK_THEME,d).apply();build();}}));headerRow.addView(menuButton,new LinearLayout.LayoutParams(dp(40),dp(40)));
        FrameLayout.LayoutParams hrp=new FrameLayout.LayoutParams(-1,dp(48),Gravity.TOP);hrp.topMargin=dp(12);hero.addView(headerRow,hrp);root.addView(hero);
