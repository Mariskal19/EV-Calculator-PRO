...
public class PrincipalActivity extends Activity {
    ...
    private void build(){
        ...
        ScrollView scroll=new ScrollView(this);scroll.setFillViewport(true);scroll.setClipToPadding(false);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(0,0,0,dp(16));scroll.addView(root);frame.addView(scroll,new FrameLayout.LayoutParams(-1,-1));setContentView(frame);
        // Keep the top spacing as a real view instead of root padding so it is preserved
        // when the theme is switched and the activity rebuilds its content hierarchy.
        Space topSpace=new Space(this);root.addView(topSpace,new LinearLayout.LayoutParams(1,dp(36)));
