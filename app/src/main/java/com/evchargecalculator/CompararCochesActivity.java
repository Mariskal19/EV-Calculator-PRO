...
public class CompararCochesActivity extends Activity {
    ...
    private void rebuild(){carsRow.removeAllViews();table.removeAllViews();
        ...
        if(selectedIds.size()>=2){
            ...
            addRow("Batería", "battery", false); addRow("Tipo batería", "type", false); addRow("Autonomía WLTP", "range", true); addRow("Consumo", "cons", false);
            addSection("Prestaciones");
            addRow("Potencia", "power", true); addRow("Tracción", "drive", false); addRow("0–100 km/h", "acc", false);
            addSection("Carga");
            addRow("Carga AC", "ac", true); addRow("Carga DC", "dc", true); addRow("10–80 %", "charge", false);
            addSection("Practicidad");
            addRow("Maletero", "trunk", true); addRow("Peso", "weight", false);
            addSection("Precio");
    ...
    private void addSection(String title){TextView s=tv(title,14,blue);s.setTypeface(null,Typeface.BOLD);s.setPadding(dp(4),dp(12),dp(4),dp(6));table.addView(s,new LinearLayout.LayoutParams(-1,dp(40)));}
    ...
    private void addRow(String label,String key,boolean highlightBest){
        ...
        TextView l=tv(label,13,text());l.setTypeface(null,Typeface.BOLD);row.addView(l,new LinearLayout.LayoutParams(dp(112),dp(42)));
        double best=highlightBest?bestNumeric(key):0;
        for(String id:selectedIds){Vehicle v=find(id);TextView val=tv(value(v,key),13,text());val.setGravity(Gravity.CENTER);val.setTypeface(null,Typeface.BOLD);LinearLayout.LayoutParams vp=new LinearLayout.LayoutParams(0,dp(42),1);if(highlightBest&&isBest(v,key,best)){val.setTextColor(blue);val.setBackground(bg(dark?Color.rgb(13,36,58):Color.rgb(235,243,255),8));val.setPadding(dp(3),0,dp(3),0);}row.addView(val,vp);}
        table.addView(row,new LinearLayout.LayoutParams(-1,-2));Space s=new Space(this);table.addView(s,new LinearLayout.LayoutParams(1,dp(4)));
    }

    private double numeric(Vehicle v,String key){if(v==null)return -1;if(key.equals("range"))return v.wltpKm;if(key.equals("cons"))return v.consumption;if(key.equals("power"))return v.powerKw;if(key.equals("ac"))return v.acKw;if(key.equals("dc"))return v.dcKw;if(key.equals("charge"))return v.chargeMin;if(key.equals("acc"))return v.acc;if(key.equals("trunk"))return v.trunk;if(key.equals("weight"))return v.weight;return -1;}
    private double bestNumeric(String key){double best=-1;boolean lower=key.equals("cons")||key.equals("charge")||key.equals("acc")||key.equals("weight");for(String id:selectedIds){double n=numeric(find(id),key);if(n<0)continue;if(best<0|| (lower?n<best:n>best))best=n;}return best;}
