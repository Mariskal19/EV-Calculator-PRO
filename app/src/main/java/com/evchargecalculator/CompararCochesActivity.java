...
public class CompararCochesActivity extends Activity {
    ...
    protected void onCreate(Bundle b) {
        ...
        try {
            ...
            // Mostrar la estructura de la pantalla primero. El catálogo se carga después
            // para no bloquear la primera renderización de la Activity.
            build();
            loadVehicles();
    ...
    protected void onResume() {
        ...
        } else if (table != null && !vehicles.isEmpty()) {
            rebuild();
        }
    }
    ...
    private List<Vehicle> marketVehicles() {
        ...
        for (Vehicle v : vehicles) if (v.market.equalsIgnoreCase(selectedMarket)) o.add(v);
        return o;
    }
    ...
    private void loadSelection() {
        ...
        if (!ordered.trim().isEmpty()) {
            for (String id : ordered.split(",")) {
                id = id.trim();
                Vehicle v = find(id);
                if (!id.isEmpty() && v != null && !selectedIds.contains(id) && selectedIds.size() < 3) selectedIds.add(id);
            }
        }
        if (selectedIds.isEmpty()) {
            Set<String> s = p.getStringSet(KEY_SELECTED, null);
            if (s != null) for (String id : s) {
                Vehicle v = find(id);
                if (v != null && !selectedIds.contains(id) && selectedIds.size() < 3) selectedIds.add(id);
            }
        }
    }
    ...
    private String joinSelection() {
        ...
        for (String id : selectedIds) { if (s.length() > 0) s.append(','); s.append(id); }
        return s.toString();
    }

    private void rebuild() {
        if (carsRow == null || table == null || summary == null) return;
        carsRow.removeAllViews(); table.removeAllViews(); summary.removeAllViews();
        ...
        if (selectedIds.size() < 3) {
            ...
            empty.setOrientation(LinearLayout.VERTICAL); empty.setGravity(Gravity.CENTER);
            ...
            TextView n = tv("Añadir coche", 12, sub()); n.setGravity(Gravity.CENTER); empty.addView(n, new LinearLayout.LayoutParams(-1, dp(24)));
            empty.setOnClickListener(v -> showSearch());
            LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(dp(145), dp(150)); lp.setMargins(dp(3), 0, dp(3), 0); carsRow.addView(empty, lp);
        }
        if (selectedIds.size() >= 1) {
            addSection("Batería y autonomía"); addRow("Batería", "battery", false); addRow("Tipo batería", "type", false); addRow("Autonomía WLTP", "range", true); addRow("Consumo", "cons", true);
            addSection("Prestaciones"); addRow("Potencia", "power", true); addRow("Tracción", "drive", false); addRow("0–100 km/h", "acc", false);
            addSection("Carga"); addRow("Carga AC", "ac", true); addRow("Carga DC", "dc", true); addRow("10–80 %", "charge", false);
            addSection("Practicidad"); addRow("Maletero", "trunk", true); addRow("Peso", "weight", false);
            addSection("Precio"); addRow("Precio", "price", false);
            if (selectedIds.size() >= 2) buildSummary();
        } else {
            TextView t = tv("Selecciona un coche para mostrar sus características.", 14, sub()); t.setGravity(Gravity.CENTER); t.setPadding(dp(10), dp(18), dp(10), dp(18)); table.addView(t, new LinearLayout.LayoutParams(tableWidth(), -2));
        }
    }

    private void addSection(String title) {
        TextView s = tv(title, 14, blue); s.setTypeface(null, Typeface.BOLD); s.setGravity(Gravity.CENTER_VERTICAL); s.setPadding(dp(4), dp(12), dp(4), dp(6)); table.addView(s, new LinearLayout.LayoutParams(tableWidth(), dp(40)));
    }

    private ImageView carImage(Vehicle v) {
        ImageView image = new ImageView(this); image.setScaleType(ImageView.ScaleType.CENTER_INSIDE); image.setPadding(dp(4), dp(6), dp(4), dp(3)); image.setBackground(bg(dark ? Color.rgb(12, 29, 43) : Color.rgb(239, 245, 252), 14)); image.setImageResource(modelImageResource(v)); return image;
    }
    ...
    private android.view.View carCard(Vehicle v) {
        LinearLayout c = new LinearLayout(this); c.setOrientation(LinearLayout.VERTICAL); c.setGravity(Gravity.CENTER_HORIZONTAL); c.setPadding(dp(9), dp(9), dp(9), dp(8));
        c.setBackground(strokeBg(dark ? Color.rgb(18, 32, 45) : Color.WHITE, dark ? Color.rgb(49, 72, 91) : Color.rgb(214, 225, 237), 18));
        ImageView photo = carImage(v); c.addView(photo, new LinearLayout.LayoutParams(-1, dp(68)));
        ...
        TextView model = tv(v.model, 17, text()); model.setTypeface(null, Typeface.BOLD); model.setGravity(Gravity.CENTER); c.addView(model, new LinearLayout.LayoutParams(-1, dp(27)));
        TextView market = tv(marketLabel(v.market), 11, sub()); market.setGravity(Gravity.CENTER); c.addView(market, new LinearLayout.LayoutParams(-1, dp(25)));
        ...
        TextView year = tv(v.year > 0 ? String.valueOf(v.year) : "", 11, sub()); year.setGravity(Gravity.CENTER); c.addView(year, new LinearLayout.LayoutParams(-1, dp(21)));
        TextView rem = tv("✕  Quitar", 12, Color.rgb(210, 70, 70)); rem.setGravity(Gravity.CENTER); rem.setTypeface(null, Typeface.BOLD); rem.setPadding(0, dp(5), 0, 0); rem.setOnClickListener(x -> remove(v.id)); c.addView(rem, new LinearLayout.LayoutParams(-1, dp(31)));
        return c;
    }
    ...
    private void showSearch() {
        ...
        List<String> ms = markets(); List<String> labels = new ArrayList<>(); for (String m : ms) labels.add(marketLabel(m));
        ...
        int marketIndex = 0; for (int i = 0; i < ms.size(); i++) if (ms.get(i).equalsIgnoreCase(selectedMarket)) { marketIndex = i; break; }
        ...
        searchMarketSpinner.setOnItemSelectedListener(new android.widget.AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(android.widget.AdapterView<?> p, View v, int pos, long id) {
                ...
                selectedMarket = ms.get(pos); getSharedPreferences(PREFS, MODE_PRIVATE).edit().putString(KEY_MARKET, selectedMarket).apply(); saveSelection(); rebuild(); renderSearchResults(input, list);
            }
        });
    ...
    private int curatedPopularRank(Vehicle v) {
        ...
        String[] p = {"tesla model 3", "tesla model y", "xpeng g6", "byd seal", "kia ev3", "hyundai ioniq 5", "volkswagen id.4", "byd atto 3", "hyundai kona electric", "kia ev6", "skoda enyaq", "renault scenic e-tech", "volvo ex30", "bmw ix1", "mercedes-benz cla"};
        for (int i = 0; i < p.length; i++) if (k.equals(p[i])) return i;
        return 1000;
    }

    /** Generic catalogue hierarchy: base -> intermediate -> extended -> AWD -> Performance/GT. */
    private int trimRank(Vehicle v) {
        String s = (v.version == null ? "" : v.version).toLowerCase(Locale.ROOT).replace("-", " ").replace("_", " ");
        int r = 300;
        if (s.contains("standard") || s.contains("base") || s.contains("pure") || s.contains("core") || s.contains("essential") || s.contains("life") || s.contains("active") || s.contains("entry")) r = 100;
        if (s.contains("comfort") || s.contains("advance") || s.contains("advanced") || s.contains("evolution") || s.contains("plus") || s.contains("boost")) r = 200;
        if (s.contains("long range") || s.contains("extended") || s.contains("max") || s.contains("pro") || s.contains("premium") || s.contains("design") || s.contains("techno") || s.contains("earth")) r = Math.max(r, 300);
        if (s.contains("awd") || s.contains("4wd") || s.contains("dual motor") || s.contains("all wheel")) r = Math.max(r, 400);
        if (s.contains("performance") || s.contains("gt") || s.contains("gts") || s.contains("rs ") || s.equals("rs") || s.contains("sport") || s.contains("m performance") || s.contains("amg")) r = 500;
        return r;
    }
    ...
    private String searchLabel(Vehicle v) {
        ...
        if (v.version != null && !v.version.trim().isEmpty()) { String ver = v.version.trim().replaceFirst("(?i)^\\d+(?:\\.\\d+)?\\s*kwh\\s*", ""); if (second.length() > 0) second.append(" · "); second.append(ver); }
    ...
    private List<Vehicle> orderedSearchVehicles(String q) {
        ...
        for (Vehicle v : marketVehicles()) {
            ...
            String hay = (v.make + " " + v.model + " " + v.year + " " + v.batteryKwh + " " + v.batteryType + " " + v.drivetrain + " " + v.version).toLowerCase(Locale.ROOT);
            if (!q.isEmpty() && !hay.contains(q)) continue;
            all.add(v);
        }
        Collections.sort(all, (a, b) -> {
            int c = a.market.compareToIgnoreCase(b.market); if (c != 0) return c;
            c = a.make.compareToIgnoreCase(b.make); if (c != 0) return c;
            c = a.model.compareToIgnoreCase(b.model); if (c != 0) return c;
            c = Integer.compare(a.year, b.year); if (c != 0) return c;
            c = Integer.compare(trimRank(a), trimRank(b)); if (c != 0) return c;
            c = Double.compare(a.batteryKwh, b.batteryKwh); if (c != 0) return c;
            c = Double.compare(a.powerKw, b.powerKw); if (c != 0) return c;
            c = Double.compare(a.wltpKm, b.wltpKm); if (c != 0) return c;
            c = Double.compare(a.price > 0 ? a.price : Double.MAX_VALUE, b.price > 0 ? b.price : Double.MAX_VALUE); if (c != 0) return c;
            return a.version.compareToIgnoreCase(b.version);
        });
        return all;
    }
    ...
    private List<Vehicle> popularVehicles(List<Vehicle> all) {
        List<Vehicle> p = new ArrayList<>(); SharedPreferences prefs = getSharedPreferences(PREFS, MODE_PRIVATE);
        for (Vehicle v : all) if (curatedPopularRank(v) < 1000 || prefs.getInt(KEY_SEARCH_COUNT_PREFIX + v.id, 0) > 0) p.add(v);
        Collections.sort(p, (a, b) -> { int ca = prefs.getInt(KEY_SEARCH_COUNT_PREFIX + a.id, 0), cb = prefs.getInt(KEY_SEARCH_COUNT_PREFIX + b.id, 0); if (ca != cb) return Integer.compare(cb, ca); int ra = curatedPopularRank(a), rb = curatedPopularRank(b); if (ra != rb) return Integer.compare(ra, rb); return orderedSearchVehicles("").indexOf(a) - orderedSearchVehicles("").indexOf(b); });
        return p;
    }
    ...
    private void renderSearchResults(EditText input, LinearLayout list) {
        ...
        List<Vehicle> all = orderedSearchVehicles(q);
        if (q.isEmpty()) {
            List<Vehicle> popular = popularVehicles(all);
            if (!popular.isEmpty()) for (Vehicle v : popular) addSearchItem(v, list, input);
            TextView header = tv("Todos los vehículos", 13, blue); header.setTypeface(null, Typeface.BOLD); header.setPadding(dp(18), dp(12), dp(18), dp(6)); list.addView(header, new LinearLayout.LayoutParams(-1, dp(32)));
            for (Vehicle v : all) if (!popular.contains(v)) addSearchItem(v, list, input);
        } else {
            for (Vehicle v : all) addSearchItem(v, list, input);
            if (all.isEmpty()) { TextView none = tv("No se encontraron vehículos", 14, sub()); none.setGravity(Gravity.CENTER); none.setPadding(dp(12), dp(20), dp(12), dp(20)); list.addView(none, new LinearLayout.LayoutParams(-1, dp(60))); }
        }
    }

    private void addSearchItem(Vehicle v, LinearLayout list, EditText input) {
        TextView item = tv(searchLabel(v), 14, text()); item.setGravity(Gravity.CENTER_VERTICAL); item.setPadding(dp(18), dp(7), dp(18), dp(7)); item.setBackgroundColor(list.getChildCount() % 2 == 0 ? (dark ? Color.rgb(18, 30, 42) : Color.WHITE) : rowAlt()); item.setOnClickListener(x -> { selectedIds.add(v.id); SharedPreferences prefs = getSharedPreferences(PREFS, MODE_PRIVATE); prefs.edit().putInt(KEY_SEARCH_COUNT_PREFIX + v.id, prefs.getInt(KEY_SEARCH_COUNT_PREFIX + v.id, 0) + 1).apply(); saveSelection(); rebuild(); Object tag = input.getTag(); if (tag instanceof AlertDialog) ((AlertDialog) tag).dismiss(); }); list.addView(item, new LinearLayout.LayoutParams(-1, dp(54)));
    }
    ...
    private String value(Vehicle v, String key) {
        if ("battery".equals(key)) return v.batteryKwh > 0 ? fmt(v.batteryKwh) + " kWh" : "—";
        if ("type".equals(key)) return empty(v.batteryType);
        if ("range".equals(key)) return v.wltpKm > 0 ? String.format(Locale.US, "%.0f km", v.wltpKm) : "—";
        if ("cons".equals(key)) return v.consumption > 0 ? fmt(v.consumption) + " kWh/100 km" : "—";
        if ("power".equals(key)) return v.powerKw > 0 ? String.format(Locale.US, "%.0f kW", v.powerKw) : "—";
        ...
        if ("charge".equals(key)) return v.chargeMin > 0 ? String.format(Locale.US, "%.0f min", v.chargeMin) : "—";
        if ("trunk".equals(key)) return v.trunk > 0 ? String.format(Locale.US, "%.0f L", v.trunk) : "—";
        if ("weight".equals(key)) return v.weight > 0 ? String.format(Locale.US, "%.0f kg", v.weight) : "—";
    ...
    private void buildSummary() {
        ...
        TextView selected = tv("Vehículos comparados", 14, blue); selected.setTypeface(null, Typeface.BOLD); selected.setIncludeFontPadding(true); selected.setPadding(0, dp(4), 0, dp(4)); card.addView(selected, new LinearLayout.LayoutParams(-1, dp(36)));
        ...
        LinearLayout commentCard = new LinearLayout(this); commentCard.setOrientation(LinearLayout.VERTICAL); commentCard.setPadding(dp(16), dp(14), dp(16), dp(14)); commentCard.setBackground(strokeBg(dark ? Color.rgb(17, 31, 44) : Color.WHITE, dark ? Color.rgb(43, 64, 82) : Color.rgb(218, 228, 239), 18));
        TextView commentTitle = tv("Comentario", 16, blue); commentTitle.setTypeface(null, Typeface.BOLD); commentTitle.setPadding(0, 0, 0, dp(6)); commentCard.addView(commentTitle, new LinearLayout.LayoutParams(-1, dp(30)));
        TextView comment = tv(summaryComment(), 13, sub()); comment.setLineSpacing(0, 1.2f); commentCard.addView(comment, new LinearLayout.LayoutParams(-1, -2));
        LinearLayout.LayoutParams commentLp = new LinearLayout.LayoutParams(-1, -2); commentLp.setMargins(0, dp(10), 0, 0); summary.addView(commentCard, commentLp);
    }

    private void addSummaryWinner(LinearLayout parent, String label, String key, boolean higherBetter) {
        List<Vehicle> chosen = new ArrayList<>(); for (String id : selectedIds) { Vehicle v = find(id); if (v != null) chosen.add(v); }
        Vehicle bestV = null; double best = Double.NaN; for (Vehicle v : chosen) { double n = numeric(v, key); if (Double.isNaN(n)) continue; if (Double.isNaN(best) || (higherBetter ? n > best : n < best)) { best = n; bestV = v; } }
        if (bestV == null) return; String val = value(bestV, key); TextView row = tv(label + "  ·  " + bestV.make + " " + bestV.model + "  →  " + val, 13, text()); row.setPadding(0, dp(4), 0, dp(4)); parent.addView(row, new LinearLayout.LayoutParams(-1, dp(30)));
    }

    private String summaryComment() {
        List<Vehicle> chosen = new ArrayList<>(); for (String id : selectedIds) { Vehicle v = find(id); if (v != null) chosen.add(v); }
        if (chosen.size() < 2) return "";
        Vehicle bestRange = bestVehicle("range", true), bestCons = bestVehicle("cons", false), bestPrice = bestVehicle("price", false); StringBuilder out = new StringBuilder(); out.append("En conjunto, ");
        ...
        out.append(". La elección final dependerá de si priorizas autonomía, eficiencia, prestaciones, velocidad de carga o precio."); return out.toString();
    }
