...
public class CompararCochesActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){
        ...
        try{
            ...
            e.setGravity(Gravity.CENTER);e.setPadding(dp(24),dp(24),dp(24),dp(24));e.setBackgroundColor(Color.rgb(8,34,58));setContentView(e);
        }
    }
    ...
    private void loadAssetVehicles(String asset,boolean required){
        try(InputStream in=getAssets().open(asset);BufferedReader r=new BufferedReader(new InputStreamReader(in))){
            ...
            for(int i=0;i<a.length();i++){JSONObject o=a.optJSONObject(i);if(o!=null)addOrMergeVehicle(new Vehicle(o));}
    ...
    private void normalizeVehicleList(){
        ...
        while(it.hasNext()){
            ...
            for(String part:v.version.split("/")){
                ...
                for(Vehicle other:vehicles)if(other!=v&&other.make.equalsIgnoreCase(v.make)&&other.model.equalsIgnoreCase(v.model)&&other.market.equalsIgnoreCase(v.market)&&other.year==v.year&&other.version.trim().equalsIgnoreCase(wanted)){found=true;break;}
                if(!found){allPresent=false;break;}
            }
            if(allPresent)it.remove();
        }
        Collections.sort(vehicles,(a,b)->{int c=a.make.compareToIgnoreCase(b.make);if(c!=0)return c;c=a.model.compareToIgnoreCase(b.model);if(c!=0)return c;c=Integer.compare(b.year,a.year);if(c!=0)return c;return a.version.compareToIgnoreCase(b.version);});
    }
    ...
    private boolean hasMarket(String market){for(Vehicle v:vehicles)if(v.market.equalsIgnoreCase(market))return true;return false;}
    private List<String> markets(){LinkedHashSet<String> s=new LinkedHashSet<>();for(Vehicle v:vehicles)if(v.market!=null&&!v.market.trim().isEmpty())s.add(v.market.toUpperCase(Locale.ROOT));return new ArrayList<>(s);}
    private String marketName(String code){if("ES".equalsIgnoreCase(code))return"🇪🇸 España";if("FR".equalsIgnoreCase(code))return"🇫🇷 Francia";if("DE".equalsIgnoreCase(code))return"🇩🇪 Alemania";if("IT".equalsIgnoreCase(code))return"🇮🇹 Italia";if("PT".equalsIgnoreCase(code))return"🇵🇹 Portugal";if("GB".equalsIgnoreCase(code)||"UK".equalsIgnoreCase(code))return"🇬🇧 Reino Unido";return code.toUpperCase(Locale.ROOT);}
    ...
    private void build(){
        ...
        LinearLayout marketBox=new LinearLayout(this);marketBox.setOrientation(LinearLayout.HORIZONTAL);marketBox.setGravity(Gravity.CENTER_VERTICAL);marketBox.setPadding(dp(10),dp(6),dp(10),dp(6));marketBox.setBackground(strokeBg(dark?Color.rgb(18,34,48):Color.WHITE,dark?Color.rgb(43,64,82):Color.rgb(220,229,240),14));
        TextView marketLabel=tv("Mercado",14,text());marketLabel.setTypeface(null,Typeface.BOLD);marketBox.addView(marketLabel,new LinearLayout.LayoutParams(0,dp(48),1));
        marketSpinner=new Spinner(this);List<String> labels=new ArrayList<>();for(String m:markets())labels.add(marketName(m));ArrayAdapter<String> adapter=new ArrayAdapter<String>(this,android.R.layout.simple_spinner_item,labels){@Override public View getView(int p,android.view.View c,android.view.ViewGroup parent){TextView v=(TextView)super.getView(p,c,parent);v.setTextColor(text());v.setTextSize(14);v.setGravity(Gravity.CENTER_VERTICAL|Gravity.END);return v;}};adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);marketSpinner.setAdapter(adapter);int idx=0;List<String> ms=markets();for(int i=0;i<ms.size();i++)if(ms.get(i).equalsIgnoreCase(selectedMarket))idx=i;marketSpinner.setSelection(idx,false);marketSpinner.setOnItemSelectedListener(new android.widget.AdapterView.OnItemSelectedListener(){public void onNothingSelected(android.widget.AdapterView<?> p){}public void onItemSelected(android.widget.AdapterView<?> p,View v,int pos,long id){if(pos<ms.size()&&!ms.get(pos).equalsIgnoreCase(selectedMarket)){selectedMarket=ms.get(pos);getSharedPreferences(PREFS,MODE_PRIVATE).edit().putString(KEY_MARKET,selectedMarket).apply();selectedIds.clear();saveSelection();rebuild();}}});marketBox.addView(marketSpinner,new LinearLayout.LayoutParams(dp(190),dp(48)));content.addView(marketBox,new LinearLayout.LayoutParams(-1,-2));

        TextView hint=tv("Elige 2 o 3 coches para compararlos",14,sub());hint.setGravity(Gravity.CENTER);content.addView(hint,new LinearLayout.LayoutParams(-1,dp(36)));
        ...
        table=new LinearLayout(this);table.setOrientation(LinearLayout.VERTICAL);HorizontalScrollView tableScroll=new HorizontalScrollView(this);tableScroll.setHorizontalScrollBarEnabled(false);tableScroll.addView(table,new HorizontalScrollView.LayoutParams(-2,-2));content.addView(tableScroll,new LinearLayout.LayoutParams(-1,-2));
    ...
    private List<Vehicle> marketVehicles(){List<Vehicle> out=new ArrayList<>();for(Vehicle v:vehicles)if(v.market.equalsIgnoreCase(selectedMarket))out.add(v);return out;}
    private int tableWidth(){return dp(112+145*Math.max(2,selectedIds.size()));}

    private void loadSelection(){selectedIds.clear();SharedPreferences p=getSharedPreferences(PREFS,MODE_PRIVATE);String ordered=p.getString(KEY_SELECTED_ORDERED,"");if(!ordered.trim().isEmpty())for(String id:ordered.split(",")){id=id.trim();Vehicle v=find(id);if(!id.isEmpty()&&v!=null&&v.market.equalsIgnoreCase(selectedMarket)&&!selectedIds.contains(id)&&selectedIds.size()<3)selectedIds.add(id);}if(selectedIds.isEmpty()){Set<String> s=p.getStringSet(KEY_SELECTED,null);if(s!=null)for(String id:s){Vehicle v=find(id);if(v!=null&&v.market.equalsIgnoreCase(selectedMarket)&&!selectedIds.contains(id)&&selectedIds.size()<3)selectedIds.add(id);}}}
    ...
    private String value(Vehicle v,String k){if(v==null)return"—";if(k.equals("battery")){if(v.batteryKwh<=0)return"—";String s=num(v.batteryKwh)+" kWh";if(v.usableBatteryKwh>0)s+="\n("+num(v.usableBatteryKwh)+" útil)";return s;}if(k.equals("type"))return empty(v.batteryType);if(k.equals("range"))return v.wltpKm>0?v.wltpKm+" km":"—";if(k.equals("cons"))return v.consumption>0?num(v.consumption)+" kWh/100":"—";if(k.equals("power"))return v.powerKw>0?num(v.powerKw)+" kW":"—";if(k.equals("drive"))return empty(v.drivetrain);if(k.equals("ac"))return v.acKw>0?num(v.acKw)+" kW":"—";if(k.equals("dc"))return v.dcKw>0?num(v.dcKw)+" kW":"—";if(k.equals("charge"))return v.chargeMin>0?v.chargeMin+" min":"—";if(k.equals("acc"))return v.acc>0?num(v.acc)+" s":"—";if(k.equals("trunk"))return v.trunk>0?v.trunk+" L":"—";if(k.equals("weight"))return v.weight>0?v.weight+" kg":"—";if(k.equals("price"))return price(v);return"—";}
    ...
    private String currencySymbol(String c){if("USD".equals(c))return"$";if("GBP".equals(c))return"£";if("CHF".equals(c))return"CHF";if("CAD".equals(c))return"CA$";if("AUD".equals(c))return"A$";return"€";}
    ...
    private void buildSummary(){TextView title=tv("🏆  Resumen de la comparativa",18,text());title.setTypeface(null,Typeface.BOLD);title.setPadding(0,0,0,dp(10));summary.addView(title,new LinearLayout.LayoutParams(-1,dp(38)));LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.VERTICAL);box.setPadding(dp(14),dp(12),dp(14),dp(12));box.setBackground(strokeBg(dark?Color.rgb(18,34,48):Color.WHITE,dark?Color.rgb(43,64,82):Color.rgb(220,229,240),16));int[] wins=new int[selectedIds.size()];String[] categories={"range","cons","power","ac","dc","acc","trunk","weight","price"};for(String key:categories){double best=bestNumeric(key);for(int i=0;i<selectedIds.size();i++)if(isBest(find(selectedIds.get(i)),key,best))wins[i]++;}int winner=-1,max=-1;for(int i=0;i<wins.length;i++){if(wins[i]>max){max=wins[i];winner=i;}else if(wins[i]==max&&max>0)winner=-2;}if(winner>=0){Vehicle v=find(selectedIds.get(winner));TextView w=tv("🏆 Mejor equilibrio: "+shortName(v),16,blue);w.setTypeface(null,Typeface.BOLD);box.addView(w,new LinearLayout.LayoutParams(-1,dp(30)));}else{TextView w=tv("🏆 Comparativa muy equilibrada",16,blue);w.setTypeface(null,Typeface.BOLD);box.addView(w,new LinearLayout.LayoutParams(-1,dp(30)));}addCategoryWinner(box,"🔋 Autonomía y eficiencia",new String[]{"range","cons"});addCategoryWinner(box,"⚡ Carga",new String[]{"ac","dc","charge"});addCategoryWinner(box,"🚀 Prestaciones",new String[]{"power","acc"});addCategoryWinner(box,"📦 Practicidad",new String[]{"trunk","weight"});addCategoryWinner(box,"💰 Precio",new String[]{"price"});TextView c=tv("Conclusión\n"+summaryConclusion(winner,wins),13,text());c.setPadding(0,dp(10),0,0);box.addView(c,new LinearLayout.LayoutParams(-1,-2));summary.addView(box,new LinearLayout.LayoutParams(-1,-2));}
    private void addCategoryWinner(LinearLayout box,String label,String[] keys){int[] score=new int[selectedIds.size()];for(String key:keys){double best=bestNumeric(key);for(int i=0;i<score.length;i++)if(isBest(find(selectedIds.get(i)),key,best))score[i]++;}int max=0;for(int n:score)if(n>max)max=n;if(max==0)return;StringBuilder names=new StringBuilder();for(int i=0;i<score.length;i++)if(score[i]==max){if(names.length()>0)names.append(" / ");names.append(shortName(find(selectedIds.get(i))));}TextView row=tv(label+": "+names,13,text());row.setPadding(0,dp(4),0,0);box.addView(row,new LinearLayout.LayoutParams(-1,dp(28)));}
    private String summaryConclusion(int winner,int[] wins){if(winner==-2)return"No hay un ganador único: los coches están muy igualados y la elección depende de qué características valores más.";if(winner<0)return"No hay suficientes datos comparables para establecer un ganador general.";Vehicle v=find(selectedIds.get(winner));return shortName(v)+(wins[winner]>=4?" es la opción más completa de esta comparativa.":" destaca en varias características, pero conviene revisar las categorías más importantes para tu uso.");}
    private String shortName(Vehicle v){if(v==null)return"—";String n=(v.make+" "+v.model).trim();return n.length()>34?n.substring(0,34)+"…":n;}

    private void showSearch(){if(selectedIds.size()>=3)return;final EditText input=new EditText(this);input.setHint("Buscar por marca, modelo, año o versión");input.setSingleLine(true);input.setTextColor(text());input.setHintTextColor(sub());final LinearLayout list=new LinearLayout(this);list.setOrientation(LinearLayout.VERTICAL);ScrollView scroll=new ScrollView(this);scroll.setFillViewport(true);scroll.addView(list,new ScrollView.LayoutParams(-1,-2));LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.VERTICAL);box.setPadding(dp(16),dp(8),dp(16),dp(8));box.addView(input,new LinearLayout.LayoutParams(-1,dp(52)));box.addView(scroll,new LinearLayout.LayoutParams(-1,dp(330)));List<Vehicle> available=marketVehicles();AlertDialog dialog=new AlertDialog.Builder(this).setTitle("Añadir coche · "+available.size()+" versiones · "+marketName(selectedMarket)).setView(box).setNegativeButton("Cancelar",null).create();Runnable refresh=()->{list.removeAllViews();String q=input.getText().toString().trim().toLowerCase(Locale.ROOT);int count=0;for(Vehicle v:available){if(selectedIds.contains(v.id))continue;String hay=(v.make+" "+v.model+" "+v.version+" "+v.year).toLowerCase(Locale.ROOT);if(q.isEmpty()||hay.contains(q)){LinearLayout item=new LinearLayout(this);item.setOrientation(LinearLayout.VERTICAL);item.setGravity(Gravity.CENTER_VERTICAL);item.setPadding(dp(10),dp(8),dp(10),dp(8));item.setBackground(bg(dark?Color.rgb(21,31,42):Color.WHITE,10));TextView ti=tv(v.make+" "+v.model,15,text());ti.setTypeface(null,Typeface.BOLD);item.addView(ti);TextView de=tv(v.version+"  ·  "+v.year,12,sub());item.addView(de);item.setOnClickListener(x->{if(selectedIds.size()<3){selectedIds.add(v.id);saveSelection();dialog.dismiss();rebuild();}});list.addView(item,new LinearLayout.LayoutParams(-1,dp(58)));list.addView(new Space(this),new LinearLayout.LayoutParams(1,dp(4)));count++;}}if(count==0){TextView none=tv(q.isEmpty()?"No hay coches disponibles":"No se encontraron coches",14,sub());none.setGravity(Gravity.CENTER);list.addView(none,new LinearLayout.LayoutParams(-1,dp(60)));}};input.addTextChangedListener(new TextWatcher(){public void beforeTextChanged(CharSequence s,int st,int c,int a){}public void onTextChanged(CharSequence s,int st,int b,int c){refresh.run();}public void afterTextChanged(Editable e){}});dialog.setOnShowListener(x->{refresh.run();input.requestFocus();dialog.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_VISIBLE);});dialog.show();}

    private void showDataError(Throwable t){if(table==null)return;carsRow.removeAllViews();table.removeAllViews();summary.removeAllViews();TextView m=tv("No se han podido cargar los datos de los coches.\n\n"+t.getClass().getSimpleName(),14,sub());m.setGravity(Gravity.CENTER);m.setPadding(dp(10),dp(18),dp(10),dp(18));table.addView(m,new LinearLayout.LayoutParams(tableWidth(),-2));}

    private static class Vehicle{
        String id,make,model,version,batteryType,drivetrain,market;int year,wltpKm,chargeMin,trunk,weight;double price,batteryKwh,usableBatteryKwh,consumption,powerKw,acKw,dcKw,acc;
        Vehicle(JSONObject o){id=o.optString("id");make=o.optString("make");model=o.optString("model");version=o.optString("version");year=o.optInt("year");market=o.optString("market","ES");price=o.optDouble("price",0);batteryKwh=o.optDouble("batteryKwh",0);usableBatteryKwh=o.optDouble("usableBatteryKwh",0);batteryType=o.optString("batteryType","");wltpKm=o.optInt("wltpKm",0);consumption=o.optDouble("consumptionKwh100",0);powerKw=o.optDouble("powerKw",0);drivetrain=o.optString("drivetrain","");acKw=o.optDouble("acKw",0);dcKw=o.optDouble("dcKw",0);chargeMin=o.optInt("charge10to80Min",0);acc=o.optDouble("acceleration0to100Sec",0);trunk=o.optInt("trunkLiters",0);weight=o.optInt("weightKg",0);}
    }
}
