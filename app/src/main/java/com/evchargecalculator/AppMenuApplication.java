...
public class AppMenuApplication extends Application {
    ...
    @Override public void onCreate() {
        ...
        registerActivityLifecycleCallbacks(new ActivityLifecycleCallbacks() {
            @Override public void onActivityCreated(Activity activity, Bundle state) {
                activity.getWindow().getDecorView().post(() -> installMenu(activity));
            }
    ...
    private void installMenu(Activity activity) {
        ...
        menu.setId(MENU_ID); menu.setText("⋮"); menu.setTextSize(28); menu.setTextColor(Color.WHITE); menu.setGravity(Gravity.CENTER); menu.setIncludeFontPadding(false);
        menu.setContentDescription("Menú de la aplicación"); menu.setShadowLayer(dp(activity,4),0,dp(activity,2),Color.argb(90,0,0,0));
        GradientDrawable bg = new GradientDrawable(); bg.setColor(Color.argb(140,12,22,33)); bg.setCornerRadius(dp(activity,12)); menu.setBackground(bg);
        menu.setOnClickListener(v -> showMenu(activity,menu));
        FrameLayout.LayoutParams lp=new FrameLayout.LayoutParams(dp(activity,40),dp(activity,40),Gravity.TOP|Gravity.END); lp.rightMargin=dp(activity,14); lp.topMargin=dp(activity,48); content.addView(menu,lp);
    }
