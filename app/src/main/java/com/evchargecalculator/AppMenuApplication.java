...
public class AppMenuApplication extends Application {
    ...
    @Override public void onCreate(){super.onCreate();LanguageManager.applyStored(this);registerActivityLifecycleCallbacks(new ActivityLifecycleCallbacks(){
        @Override public void onActivityCreated(Activity activity,Bundle state){activity.getWindow().getDecorView().post(()->{LanguageManager.applyStored(activity);installMenu(activity);if(activity instanceof MainActivity)installChargeMenuButton(activity);LanguageManager.translateViews(activity);});}
    ...
    private void installChargeMenuButton(Activity activity){
        ViewGroup content=activity.findViewById(android.R.id.content);if(content==null)return;
        View globalMenu=content.findViewById(MENU_ID);if(globalMenu!=null)content.removeView(globalMenu);
