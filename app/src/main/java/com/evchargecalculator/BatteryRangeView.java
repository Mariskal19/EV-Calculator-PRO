...
public class BatteryRangeView extends View {
    ...
    private int activeThumb = -1; // 0 = inicial, 1 = objetivo
    ...
    @Override protected void onDraw(Canvas c){
        super.onDraw(c); float y=getHeight()/2f; float left=dp(8), right=getWidth()-dp(8);
    ...
    @Override public boolean onTouchEvent(MotionEvent e){
        ...
        if(e.getAction()==MotionEvent.ACTION_DOWN){
            float dc=Math.abs(x-xFor(current));
            float dt=Math.abs(x-xFor(target));
    ...
    private void updateThumb(float x){
        int v=valueFor(x);
        if(activeThumb==0){
            current=Math.max(0,Math.min(v,target-1));
        } else if(activeThumb==1){
            target=Math.min(100,Math.max(v,current+1));
        }
        invalidate();
