...
public final class LanguageManager {
    ...
    static{
        ...
        add("Cargar la batería desde","Charge the battery from","Charger la batterie de","Batterie laden von","Carica la batteria dal","Carregar a bateria de"); add("al","to","à","bis","al","a"); add("Se cargará","Will charge","Charge prévue","Wird geladen","Verrà caricata","Será carregada");
    ...
    public static String t(String key,String lang){if(key==null)return null;if("EV Charge Calculator".equals(key))return key;String[] a=TR.get(key);if(a==null)return key;if("es".equals(lang))return a[0];if("en".equals(lang))return a[1];if("fr".equals(lang))return a[2];if("de".equals(lang))return a[3];if("it".equals(lang))return a[4];if("pt".equals(lang))return a[5];return a[1];}
    public static String translateDynamic(Context c,String s){if(s==null)return null;if("EV Charge Calculator".equals(s))return s;String lang=getEffectiveLanguage(c);if("es".equals(lang))return s;for(Map.Entry<String,String[]> e:TR.entrySet()){String[] a=e.getValue();for(String source:a)if(source!=null&&source.equals(s))return t(e.getKey(),lang);}return s;}
