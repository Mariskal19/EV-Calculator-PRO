...
SOURCES = ['vehicles.json', 'vehicle_variants.json', 'vehicle_market_additions.json', 'vehicle_market_additions_pass4.json', 'research_mini_ES_2024_2026.json']
...
def logical_key(vehicle):
    return '|'.join([
        str(vehicle.get('make') or ''), str(vehicle.get('model') or ''),
        str(vehicle.get('market') or ''), str(vehicle.get('year') or 0),
        battery_key(vehicle), normalized_version(vehicle)
    ]).strip().lower()
...
for vehicle in vehicles:
    ...
    for part in parts:
        if not any(
            other is not vehicle and
            str(other.get('make','')).lower() == str(vehicle.get('make','')).lower() and
            str(other.get('model','')).lower() == str(vehicle.get('model','')).lower() and
            str(other.get('market','')).lower() == str(vehicle.get('market','')).lower() and
            other.get('year') == vehicle.get('year') and
            str(other.get('version','')).strip().lower() == part
